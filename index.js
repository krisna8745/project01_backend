...
app.use(
  cors({
    origin: ["https://www.yox9.com","https://yox9.com", "https://admin.yox9.com","https://subadmin.yox9.com","https://master.yox9.com","https://superagent.yox9.com","https://agent.yox9.com"], // Replace '*' with the specific origin(s) you want to allow, e.g., 'https://yourdomain.com'
    methods: ['POST', 'GET', 'PUT', 'DELETE'], // Define allowed HTTP methods
    credentials: true, // Allow credentials like cookies to be sent
  })
);
...
// MongoDB connection
mongoose.connect(`mongodb+srv://rsssss234ss_yox99:pYSF7TEP5OpNnJ8L@cluster0.zogxllw.mongodb.net/`)
...
app.post('/api/admin/signup', async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({
      ...
      user: { id: savedUser._id, username: savedUser.username, email: savedUser.email ,userNo:savedUser.userNo,pwd: savedUser.pwd,},
    });
  } catch (err) {
    console.error(err);
    ...
    for (const marketId of marketIds) {
      try {
        const response = await axios.get(
          `https://socket.1ex99.in/v2/api/oddsDataNew?market_id=${marketId}`,
          {
            headers: {
